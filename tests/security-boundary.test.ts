...
describe("deployment docs", () => {
  test("examples do not publish Mousehole with bare 5010:5010", async () => {
    ...
    const barePortPattern = /(?<!127\.0\.0\.1:)5010:5010/;
    const matches: string[] = [];

    for (const file of files) {
      const content = await readFile(file, "utf8");
      if (barePortPattern.test(content)) {
...
async function collectDocumentationFiles(entries: string[]): Promise<string[]> {
  const files: string[] = [];

  for (const entry of entries) {
    await collectDocumentationFilesFromPath(
...
async function collectDocumentationFilesFromPath(
  filePath: string,
  files: string[],
): Promise<void> {
  ...
  if (stats.isDirectory()) {
    ...
    await Promise.all(
      children.map((child) =>
        collectDocumentationFilesFromPath(path.join(filePath, child), files),
      ),
    );
    return;
  }

  if (/\.(?:html|md|xml|ya?ml)$/.test(filePath)) {
    files.push(filePath);
  }
}
